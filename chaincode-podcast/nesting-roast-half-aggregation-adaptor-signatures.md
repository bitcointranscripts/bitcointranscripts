...
For example, for MuSig2, in a specific setting, we know we're pretty confident that this is secure.
...
And we use this, for example, in BIP 32, deterministic key derivation, we use it in Taproot, we may use it in other schemes.
...
And I think for MuSig2, that is fairly close.
...
But the whole provable security aspect is just one part and we shouldn't forget about the rest because multi-sig in Bitcoin, the threshold multi-sig naive thing, P2SH in 2012 was introduced for specifically that purpose because there was no address scheme to do multi-sig at the time.
...
And I think the conversation about diving into the advanced signing protocols and multi-sigs versus threshold signatures and state of FROST and ROAST and MuSig2, I thought that was all really fun.
