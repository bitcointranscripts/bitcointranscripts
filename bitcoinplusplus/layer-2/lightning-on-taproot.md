...
The one of the two big improvements that `Taproot` brings is Schnorr signatures and the cool thing about Schnorr signatures is that they allow us to have quite trivial n of n signatures that still look like they're single SIG.
...
So let's say that we have Alice and Bob, who are trying to open a channel between the two of them and specifically, they're trying to figure out what their shared public key is gonna be.
...
The important thing is just that you know which public key corresponds to which coefficient and then, say if you're Alice and your pub key also comes first, so your index is 1, because in math, indices don't start at 0.
...
That thing is hashed, and that hash is concatenated with her own pubkey, then we hash that again, and so, because of elliptic curve photography, we can simply interpret hashes as big integers, we take that hash and simply use that as a big integer that is a coefficient for her public key.
...
And one might actually wonder, why, if we're already doing this stuff with keys, Do we even have to bother with non-segregation and adding coefficients to our nonces?
...
**Arik Sosman:** 00:12:51

You could fool people into nonce, yeah.
That is one thing and you could also construct a nonce adversarily in such a way that you would then be able to unilaterally create a signature with `MuSIG2`.
...
We have that automatically from the addition that happened beforehand and The coefficient we're using is actually the sum of the nonces with index 1, the sum of the nonces with index 2, the aggregate public key that we have already determined in the prior step, which we only need to do once, and the message that we want to assign.
So then it's just a matter of aggregating all of those partial signatures with those nonces.
But yeah, really the interesting trick, the cool innovation with `MuSIG2` is the fact that each participant sends two nonces at once, and that we then aggregate the nonces by their index across all the participants.
However, even in MuSIG2, you would think, all right, now we have this thing where we are calculating coefficient, MuSIG2 is guaranteeing that there should be no nonce reuse.
...
So here I have simplified the equation a little bit, because really what we would get is, this expression at first, the hash multiplied with a private key, plus, in principle, it ought to be C prime, which times rA, and then plus rB, because we have those two.
...
I'm pretty sure everybody can tell, is a very simple linear equation system.
...
And the way it works is when Alice opens a channel, she doesn't even bother in theory sending any `nonce` whatsoever because she doesn't even know whether Bob's going to accept the channel yet.
So why generate any sort of cryptographic material if you're never going to end up using it.
So if Bob however does opt to open a channel with Alice and accept it, then he can generate a local nonce pair for himself and he then sends his own local nonce, which is still a pair, we have to remember that now even though it's called a nonce, it's still a nonce pair because of the whole music 2 thing.
...
So one of them is if we are able to provide the hash preimage, which is marked in green here, And the script looks a little complicated, but the point is, once `OP_CHECKSIG` verifies here, then it should be 1, and then we have `OP_CSV`.
But if `OP_CHECKSIG` does not verify, then we have a 0, and then we have 0 `OP_CSV` and The question is, would it then mean that without a valid signature, we would be able to spend it without delay?
...
## The Gossip Protocol & PTLCs

**Arik Sasman:** 00:31:02
...
**Arik Sasman:** 00:34:44

So if you're monitoring the gossip from lightning, then you would know the particular output on-chain of course corresponds to the Lightning channel.
...
Even today, the odds of multiple hops being correlated on chain are fairly low because you have to have multiple channels go on chain with `HTLC`s in flight, which, let's be honest, is not a big risk.
...
However, how then is Bob able to send a `PTLC` to Carol or how is Carol able to send a `PTLCs` today?
The interesting thing is that the `PTLCs` that Bob sends to Carol in the Onion message also contains the secret number that Alice generated for Carol.
Bob doesn't know it because it's in the onion, but Carol does.
And that is where I really struggled with the visualization.
So the `PTLC`s are truly peer-to-peer, but this thing that is originating from the very beginning, those lowercase b, c, d, and the sum here, those are meant to be in the onion packet that Alice sends.
...
And then by subtracting their own secret, which Dave still has because it is lowercase D that Dave originally received in the onion packet from Alice, Dave is then able to subtract lowercase D and using that, unlock the `PTLC` from Carol.
...
An adapter signature looks almost exactly like a Schnorr signature, except it is invalid.
It is tweaked.
As you can see here, we have, if we ignore the T here, we have exactly the thing that we would expect from a Schnorr signature, but our commitment in the hash is not to all random nods, but our random nonce plus some tweaked T.
...
So therefore, a plus b plus c plus z is our tweak, which means that then, once we find out the valid signature later on, that tweak is going to be lowercase a plus b plus c plus z.
...
Why do we have to have music just to create an adapter signature that is tweaked damaged broken Whatever terminology when I use.

**Audience:** 00:47:08

The same reasons you don't want the nodes private key leads, you don't want the payment point linked as well, if you're not doing the same knots, you know, computations or anything.
Just depending on that payment not to be there in general.

**Arik Sasman:** 00:47:24

what would happen if it were to be ?
...
Do the nonce need as an input the `PTLC` point transfer?
Can you sort of pre-share, like here's the next 10 nonce and you can do pre-sharing.

**Arik Sasman:** 00:51:17
...
But that is exactly the sort of thing that I think wants research.
