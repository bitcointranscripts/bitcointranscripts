...
---
**Maurice Poirrier:** 00:00:02

So, hi, I'm Maurice, I'm an LNDK maintainer.
I'm going to explain what it is now soon.
I'm a Spiral grantee and also part of the B4OS/Lib Satoshi team.
...
We could use recurring payments in the future, or we can add other stuff on the spec so it can be done using this and actually as Aliyah's talked before, actually fixes the privacy and censorship problem that current has because of Onion messages, does not rely on HTTP servers like LNURL, that they can be censored, and also root blinding, let the receiver do not show herself when in a payment or when they are receiving Onion messages.
...
And the receiving node is in the end and it could be like the same node itself repeating itself and not being any forwards on that or this can be like different nodes that they are forwarding like only messages between them.
...
And if we go to the Elias's presentation, it's 5.6% of node supports on your messaging.
...
So it's a shim because it intercept calls from the LND side, translate it, do something about it and then it's going to be used in the LNDK back and forth.
So LND is mostly like a messenger in this case.
LNDK does the heavy part of signatures and everything related.
...
What's next?
We already built the BIP353 payments and Merck can pay user flows that are coming in the next version.
Afterwards, we don't know yet what's going on about the project.
So with that, currently with the latest version, we can have all the instructors, all the TLVs, and we can do all the payment flows of BOLT 12.
So that's a great win for the network itself.
So instead of just saying that words of math, maybe it's like use LNDK with your LND node and you can use BOLT 12.
...
First, We need to, on the spec of BOLT 12, you have to do some signatures, not the offers, but all the invoice requests and the invoice needs to be like signed.
...
So we did, we stole the idea from the BIPs, BIP 50 actually, that they use this kind of message that is going to be signed in LND with a specific seed key found in a specific key index.
...
But with this, we could actually restart the server with no state.
...
And we're going to use the same custom message in the LND.
...
I'm spinning up LND, Eclair 1, Eclair 2, and CLN network, it's going to create the channels automatically.
...
So live demo.
So we have all the node info and already copied the secrets wherever I needed to.
So if we run LN-cli list.
...
We are not using the invoices in BOLT 12 doesn't have a standard way of showing off.
We are just using the hex string.
And if we want to just pay the offer that is going to fetch the invoice and then we just pay it. 
It's going to do exactly the same.
...
They were first doing, adding on the TLV metadata, where We were adding a nonce and we could calculate using the TLV records and HMAC also added an encrypted payment ID so we can query faster and using this, we could actually maintain the statelessness and not store in a database which one were our offers.
...
We receive a gRPC, we start building our blinded path, for that we list our channels, because channels are the most conservative way of connecting to peers, so we use that because offers can be like long, they can last longer.
...
And whenever we receive an invoice request through the Onion message, what we do is when queue the Onion processing, the onion processor will know that we are receiving this, they are asking for an invoice, we should authenticate the invoice request with the trick that I already told you, we then create an invoice for the invoice request, we use, actually we create a BOLT 12, BOLT 11 invoice using a blinded path options in LND, then using that we parse the BOLT 11 and just transform it in what is a BOLT 12.
...
For a CLN they are doing it differently, you have to request the invoice and then pay it, So I'm going to do that.
And there you go.
...
And wrapping up, so BOLT 12 is important for Lightning Privacy, Sensory Street Precision and also better UX.
And LNDK is important in the ecosystem so we can accomplish that.
You can run it now.
...
**Maurice Poirrier:** 00:21:18

Yeah, it's actually we don't know yet.
For the first part we will keep maintaining but we are thinking to add also other features that are not in LND for example async payments is a thing that we are currently looking at and we probably continue on that path for the next year or so.
...
**Maurice Poirrier:** 00:21:54

Yes, so, that one was one of the main issues in the beginning for strike, the strike is running this, so we added a rate limit, rate limiter inside the LNDK, so we are trying to, if we have so many message from one peer, or two one peer, we're actually rate limiting inside of the queue, and the queue also has a specific fixed size of it, so we haven't had that many issues related to the OS attacks.
...
Is my first question, the second one, are you considering doing splicing or anything like that, that LND doesn't have as well?
No, that's a big one.

**Maurice Poirrier:** 00:22:56
