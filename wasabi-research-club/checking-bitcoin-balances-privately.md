...
There's another solution in `BIP157` and `BIP158`.
...
The blocks are one megabyte up to one megabyte each and for kind of privacy reasons, you can't just get a subset of the block.
...
So one problem is the act of fetching the block is not protected, we announce which block we're trying to fetch.
But what we do as a mitigation, if we did this all with the same node, that would be bad because the node could kind of pretty easily tell from the blocks that you fetch which address you're interested in.
...
But you know again here the timing really trips us up right again just knowing that just watching it even if I'm not like even if I'm not a `Tor` adversary, I'm not able to de-anonymize you on `Tor`.
...
So if the database gets bigger, so today the database is a bunch of Bitcoin balances, if we added transaction data or if we want to include more data say about individual transactions, that will make the server's runtime longer.
...
But before I do that, I'll just kind of say, yeah, I think the open questions for us are what minimum set of data is enough?
...
So you can think of computation as just a cost, if you did it with three cores, you would take one second.
...
So the only way that you're going to have an incorrect balance is if the server somehow malleates a filter but there's not really good, it's kind of a weird, unclear attack vector.
...
And yeah, to be clear, today we definitely do not have any proof that the server is really serving you the right data.
...
I'll highlight one alternative way of doing this, which is actually suggested in the BIP157.
...
Yeah, it's a large outgoing cost, I'm sure your hosting provider is charging you.
Actually doing proofs of inclusion is possible, it's good to know you need that kind of to deploy this for real.
If I can ask a follow-up question, do you guys, to do this, are you mostly querying the UTXO set, the full set of transactions, just balances?
Like what kind of data is crucial?

**Max Hillebrand:** 00:19:30

We do want the full transaction history list and so that's what we get in the filters right now.
Wasabi is `SegWit` only, so we don't have to create filters `pre-SegWit`, August 2018 or something, or 17.
...
So they're very compact, three years ago, because it was a minority of people use those addresses more and more they become larger and larger but they're they're they're very space efficient I don't know the exact details maybe Max can answer but the exact

**Max Hillebrand:** 00:20:31
...
I have tried to see the information on the internet, but it's just not possible for a basic layman like me.
...
I guess when I'm thinking about it, I suppose it helps that you guys only support the `bech32` addresses, because That means there's kind of a limited set of addresses that are on chain that have transactions for that.
...
So, the problem that you guys face with block filters is mostly that you have to download this gigabyte of things to get started, is that the issue?

**Max Hillebrand:** 00:24:08
...
I see the issue.
Yeah, so there, weeks if not a month.

**Audience 1:** 00:24:57

So, let me just add one more thing.
So when you use Wasabi Wallet, you're making many addresses because you're CoinJoining so you have to make a new address every time and if you use the wallet regularly you might go through something like 300 addresses or 400 or thousand over time that means that what you're looking for in the filters is actually quite broad.
...
So it's really useful to hear that a very typical use case is like hundreds of addresses, because that says a lot about how we need to build this to make it usable.
Yes, so batching is possible, but it's in the works.

**Max Hillebrand:** 00:27:56

Yeah, by the way, it's way more than just a thousand.
I'm checking a not even that old wallet and it has well 8,000 addresses here 13,000 addresses so since we do you know we do we attempt many CoinJoins and a lot of them fail and we generate new addresses for each attempted coin join and you can register up to 8 outputs in a round so let's say a round fails, I don't know, 5 times before it succeeds so that's 5 times 8 addresses that we have to add to your gap limit.
...
And Wasabi Wallet already handles the mempool, right?
...
**Max Hillebrand:** 00:33:59

And Does that size depend on the database size?

**Samir Menon:** 00:34:02

It does not or it only logarithmically does.
...
So, so I think that the way that it's not so that's very true.
...
I mean, the fact that you don't incrementally have to do anything, like each query costs the same and, or I guess the first one costs a little more, but basically there's no syncing, right?
...
**Samir Menon:** 00:38:18

I don't know if that's a feature.
Yeah, I don't know if it's a feature you guys are interested in, but I think it would be useful to be able to just say, like how much is at this address?
Because today, the option is to go to a Chrome tab and type it in and send it to blockchain.com or whoever.

**Max Hillebrand:** 00:38:40
...
**Samir Menon:** 00:40:00

You know that it's less than or equal to, but you don't know what gap was left for the minor in the form of the fee.

**Max Hillebrand:** 00:40:08

Exactly and then you can do child pays for parent fee calculations.

**Max Hillebrand:** 00:40:17
...
**Max Hillebrand:** 00:41:12

Definitely.
Oh yeah, by the way, to jump back.
So you say the client needs to upload his phone public key to the server.
So then all the queries are connected to the same public key?

**Samir Menon:** 00:41:26
...
**Samir Menon:** 00:44:12

The client has to wait for as long as it wants to, almost directly in proportion to how much it's willing to pay.
So I mean, if you want to make a thousand queries, right, let's say it's 5000 seconds of computation, right?
...
So, it is kind of, it is infinitely divisible.
It's naively parallel as an algorithm.

**Max Hillebrand:** 00:45:01

That's really interesting to hear.

**Samir Menon:** 00:45:06
...
**Max Hillebrand:** 00:45:19

Okay, I think I'm pretty much out of questions for now.
Does anyone else have any?
No, then would be nice to get a bit more into the crypto magic, if you could?

**Samir Menon:** 00:45:30

I'm If this altitude is kind of too much or too little, let me know.
...
So I'll explain, the encryption we use has a special property, which we'll get to, but basically it is also, it behaves mostly like a normal encryption scheme.
So you encrypt each bit, and now these encrypted bits get sent to the server.
...
**Max Hillebrand:** 00:49:27

Nice.
Thanks.
Continue,

**Samir Menon:** 00:49:29
...
And that makes the query smaller.
...
If there was an encryption scheme that had this multiplication and this addition operation, then it would all work.
...
There are a bunch of things you can do to kind of not actually send encrypted zeros.
So I guess I can just explain it this way.
So the operations we have are homomorphic multiplication by a plaintext item and addition.
I don't know your familiarity with linear algebra or something, but that's a not quite complete set.
If you were also able to multiply two encrypted numbers.
...
So if I give you A, S and I give you A, it's easy to find S, because finding the inverse of a matrix is not hard.
But the key insight, the algebraic insight that enables this is that, Well, if I give you A S plus E, where E is just some very small noise, suddenly the problem is very hard.
...
**Samir Menon:** 00:57:55

Yeah, it's a good question.
So for the noise we use is sampled from the Gaussian distribution, so like a normal distribution.
...
**Samir Menon:** 01:03:40

That's a good question.
I mean, it used to be impossibly big.
It used to be like, I would say years to do a single Bitcoin address lookup.
...
I mean, come on, like seriously, if this is the issue and we could buy a server for it, like I think it would be worth it because, this is the single most problematic thing that we have performance-wise in Wasabi Wallet.
...
It's not perfect, but something I'll do is I think I will l try to build up a number of addresses in my wallet that's large enough to kind of test with.
...
So yeah, I don't know if you guys are interested.
...
But then when you want to just like know how much money you have, you have to just, you end up kind of just going to a block explorer or ledger or whoever, and kind of just telling them your address and your IP address and stuff.
...
**Samir Menon:** 01:11:53

Especially from mobile clients and stuff.
Yeah, no, go ahead.

**Max Hillebrand:** 01:11:58
...
**Samir Menon:** 01:13:00

Yeah, I think another way to think about it would be, if you want to just kind of like not do the management, but get all the privacy, I think this is the way to do it.
...
**Samir Menon:** 01:15:30

No, not really.
I think you'll run into memory at some point.
I mean, you'll start thinking, if you put 5000 cores on a system with 8 gigs, you might get bottlenecked by other things.
...
You kind of explained that they're kind of the outputs of a, their output addresses as a result of a CoinJoin, sometimes a failed CoinJoin.
But yeah, the more I can kind of understand that, the more I think it could be feasible kind of to make progress.

**Max Hillebrand:** 01:17:12
...
Mike, you are the one who wrote the `Golomb-Rice filter` stuff, so you put a lot of thoughts into around here.

**Lucas Ontivero:** 01:18:37
...
It's very good.
So I think it, no, I think it doesn't worth the effort to even if the requirement of the technology in terms of bandwidth, storage, CPU, even if we can afford that, I don't know if we can afford to change the architecture of the wallet to use this technology, right?
...
**Samir Menon:** 01:22:38

Your point about the multiple servers is funny.
...
I mean, if you use Wasabi wallet, but then use a public block explorer, you're kind of fucking it up, you're kind of not, you're kind of defeating some of the purpose of the wallet.
So I hope at least those two things sound kind of complimentary.

**Lucas Ontivero:** 01:23:28
...
So I can think of like in the kind of Ethereum version, I can think of like, some NFT thing, they like, pull up some image there's like that kind of data.
But what are you thinking of that is global that these explorers would show?

**Lucas Ontivero:** 01:25:28
...
**Samir Menon:** 01:25:35

Ah, I see.
So what IP submitted this address?
Are there, there's also, is there like a kind of naming, is there like a DNS equivalent, like people who use like OP_RETURN to kind of store data in some kind of structured way to register a name or whatever.
That kind of data is also, while it's in the transaction, the metadata about it is not, right?

**Max Hillebrand:** 01:26:12
...
My best my biggest thing I always make fun of is like, I think IBM had like a big demo where you could look up like the 50 US states privately.
...
So we found ways to exploit the fact that with fully homomorphic encryption, you can compute any function on encrypted data.
...
There are now a couple, but at the time we were kind of one of the early ones that, yeah, I mean, when we told people, you go to this site, and we could read Wikipedia, that was like a big deal, because I think everything else had been kind of speculative.
...
**Max Hillebrand:** 01:30:39

yeah the MVP demos are pretty great

**Samir Menon:** 01:30:44

yeah if you have ideas for other ones let me know I want to do Ethereum and I want to do, sorry, a password checker.
...
But just curiosity, how do you see the homomorphic encryption?
How far do you think we are from fully, for example, can operate in a remote on data, on encrypted data in a remote, I don't know, for example, I save something in Google, in Google Drive,
...
And it'll always be like 100 times more expensive than doing it the regular way.
...
**Max Hillebrand:** 01:34:52

It's pretty sweet.
Just a fun fact.
But the Signal groups use the key to verified anonymous credential cryptography scheme that we use for our Wasabi CoinJoins.
So it would be nice if we throw your homomorphic encryption on top.

**Samir Menon:** 01:35:10
...
**Samir Menon:** 01:35:29

Okay.

**Max Hillebrand:** 01:35:30
...
But it's something I mean, I don't know exactly how to define homomorphic encryption, right?

## Homomorphic Encryption

**Lucas Ontivero:** 01:37:51
...
**Samir Menon:** 01:38:36

It's funny, I think elliptic curves are way more complicated.
I'm always like, I think lattice-based cryptography and homomorphic stuff, which is lattice-based and was initially pioneered for post-quantum resistance and stuff.
...
Whereas lattices it's matrices and it's noise.
...
**Max Hillebrand:** 01:39:48

Self-taught cryptographers.
The most dangerous kind.

**Lucas Ontivero:** 01:39:55

Oh, yes.

**Samir Menon:** 01:39:56
...
If the implementation on FPGAs or ASICs could constitute a significant improvement.

**Samir Menon:** 01:45:58
...
**Max Hillebrand:** 01:47:05

I show you a question because you just said the database has to be kept in memory.
Like, is that the case in Spiral?

**Samir Menon:** 01:47:11

Yes.

**Max Hillebrand:** 01:47:12

Oh, okay.
But then, I mean, if we have a large data data set, for example, the full blockchain, many hundreds of gigabytes.
Right.
All of that needs to be in memory?

**Samir Menon:** 01:47:23

Yeah, yeah, it does.
I will say the memory cost usually kind of pales in terms of in comparison to the compute cost.
So I mean, making everything fit in memory is, I mean, hundreds of gigs of memory is on a cloud provider, not that expensive, actually.
...
And lately, there has been some developments regarding open source tooling for like Yosys and stuff.
...
In fact, I think the white paper is not a good starting point. 
So, sometimes you need to know the tools that you have available.
...
**Lucas Ontivero:** 01:53:17

Okay.
Thank you guys.

**Max Hillebrand:** 01:53:20
...
**Samir Menon:** 01:53:40

Yeah, absolutely.
Thank you so much.

**Max Hillebrand:** 01:53:43

See you on the next show.
Bye-bye.

**Samir Menon:** 01:53:44
