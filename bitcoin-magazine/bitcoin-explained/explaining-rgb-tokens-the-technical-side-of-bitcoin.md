...
...everybody would have the image and you could take the hash of the image and then the hash of the image would be on the blockchain.
And so-
...
Yeah, so the coins, the Satoshis, were colored, basically.
So it's where the naming comes from, right?

Sjors Provoost: 00:17:57

Actually.
And this is very very brittle because you need to use a special wallet that understands these coins and keeps them separate.
...
Right.
So that's the big difference compared to colored coins, a counterparty, is that now we have a separate layer..
...
Why do we want this on Bitcoin?
