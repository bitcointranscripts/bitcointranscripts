...
source_file: https://anchor.fm/s/12fe0620/podcast/play/71028929/https%3A%2F%2Fd3ctxlq1ktw2nl.cloudfront.net%2Fstaging%2F2023-4-25%2F8ab73b8d-a824-fc44-fa03-8ce523cb5d4f.mp3
...
And one of the pain points of green was that it was, it supported only a few, or still it supports only a few types of scripts, which is like two of two, two of three, stuff like that.
...
Like, tell me the progression of how the project evolved and maybe Daniele, you can fill us in on where it is today.

Speaker 3: 00:03:19

Daniele Della Puglia Yeah, so I mean I think that the biggest part of the project was developed in the first few months and then it became more of a just fixing it up and improving it little by little.
...
Yeah, I think, I mean, I did the coin selection, which Merch reviewed and the code was horrible.
...
Because, yeah, basically Alex just told me, hey, we need more people contributing on the library.
...
And the BDK wallet is not thread safe, so you have to work around it and use new text and stuff.
...
The other reason is we probably don't have the right bindings because right now we have Zwift and Kotlin, these are the two main ones, because we wanted to target mobile users.
...
And if you're building a wallet, I mean, you should know something about Bitcoin, of course, but you don't have to know exactly what's a C-cash and what's a lock time, etc.
...
There was the one of two or two of two multi-sig with a decay and I think there was also a 2 of 3 wallet at some point maybe?

Speaker 3: 00:13:36

Yeah so Green is interesting because it's a very old product, old in terms of Bitcoin years, Bitcoin age and so it kind of started in a way and then kind of evolved with Bitcoin where OPCSV at some point became available so they started using it.
So internally green supports the old 2 of 2 it's like it's a normal 2 of 2 but they email you a pre-signed transaction with a future alloc time.
...
Then they introduce the two of two with CSV.
So that's done inside the script, it's the user always need to sign, and then it's either the server or you wait some time.
And then I think they introduced a slightly tweaked version of this concept, which is still a two of two with CSV, but it's descriptor compatible, essentially.
So it's basically written, the descriptor is a form that can be parsed into a descriptor or vice versa.
You can from a descriptor get the script and then they also have two of three, which doesn't have any time lost or anything.
...
So if you have a wallet and you say this is going to be this forever, no top root, no next top root in five years when it comes, then okay, stay with your code.
...
Wizard Sardin, yeah.
Which is the company behind Revolt.
...
Yeah, I don't want to promise anything but I think that eventually the BDK team might work on that just because in BDK we do support AdWords wallets using Rust HWI as Alikos was saying, but every time we have some kind of problem and it's Python related, we just look at the code and we're like, come on, let's just start it again from scratch, right?
...
So right now in BDK, we do have one REST trait, which defines how you sync the wallet and just for context we have this structure called wallet and what it has inside it's the transactions of the user and the UTXO so that you can just build new transactions, right?
So, sync basically means either you go to a service like Explorer, Electrum, or Bitcoin Core RPC, or maybe you use compact block filters, and somehow you update your internal state.
...
The first one is some network call, let's say, where you just go to Explorer, go to Electrum, go to Bitcoin Core RPC, and just say, hey, what do you have for me?
...
the Electrum server receives from the same connection five requests, can assume that those five scripts are from the same user.
So you can kind of correlate scripts.
And I guess they wanted to build separate total circuits so to the electronic server it looks like five different users asking for one script each.
...
Exactly, you can have some weird heuristic on how to open the connections, et cetera.
...
Basically, VDK is super generic, so you can have time locks and complex conditions in your script.
...
You can say I have this assets available, which means I have this key available so I can produce signatures with this key and another asset is I'm willing to wait so you can use time loss as well.
...
And so the planning module can figure out and say, oh, yeah, you can spend just using this key if you wait a little bit and you can kind of interactively construct a plan to spend and then you can construct a transaction using the plan so it will set the write and sequence a lot of time and everything.
So yeah, that's another thing that's coming.

Speaker 2: 00:29:45

Yeah, I'm pretty hyped about that as well.
So basically for using that, we have to basically rewrite the transaction builder API, because instead of building a transaction just saying, I want to use this policy, you have to provide assets, et cetera.
...
The music too, BIP has been merged now.
