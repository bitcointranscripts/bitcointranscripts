...
I think that’s not really other, but using the trace points to review PRs has helped me a lot. Actually, for example, looking at some P2P changes and actually seeing the protocol change here a bit, or it works the same as before and so on. And that actually for me personally has helped because I’m not the guy that sits in front of the C++ code and reads it all day. I’m more the guy that looks at it visually, for example, or in some way, filter it, looks at what’s happening and not what should happen.
