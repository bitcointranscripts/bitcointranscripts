...
transcript_by: 'Hellman-77 via review.btctranscripts.com'
...
Your devices but I guess mostly used to not charge the hardware wallet but just to turn it on and keep it on. -->

**Aaron van Wirdum:** 00:01:05

<!-- Anyways, another amazing CoinKite product and here's another one, the `Coldcart Q`.
Josh has got many Cold Cart, CoinKite devices, buy them all. -->
Okay, let's get to our episode. We're going to discuss `silent payments`.

## Speaker Introduction

**Sjors Provoost:** 00:01:26

That's right and we talked about that in episode number ?

**Aaron van Wirdum:** 00:01:31

52 from the top of my head, actually.

**Ruben Somsen:** 00:01:33

Somewhere around there. I don't remember exactly, but it was somewhere 50-something.
...
Yeah, we'll give our dear listener, you, our dear listener, a short update but first, let's do a short introduction also.
...
But you could kind of see it as a second layer because it's a sort of secondary way of interpreting data on the Bitcoin blockchain. So sure, let's say.

**Aaron van Wirdum:** 00:02:25

No, no, no.
I'm just giving a picture of who you are usually, but this is a different proposal for you.
Josie, our listeners don't know you yet. Who are you?

**Josie:** 00:02:36

I am a Bitcoin `Core developer`, started contributing about three years ago,
I started off in the wallet mostly out of an interest in privacy stuff, and also just usability, because I used the Bitcoin Core wallet, which was part of what got me so excited about `silent payments`. It was kind of the feature that I felt like was missing, but yeah, I spent a lot of time in the wallet and also a lot of time thinking about privacy improvements without `Soft-Forks`.

**Aaron van Wirdum:** 00:03:02

So what happened? At some point you read Ruben's proposal and you decided to implement it or what?

**Josie:** 00:03:07

No, I heard him present on it at `TabConf` in October of 2022.
...
This sounds awesome and then I think at that point, we decided the next step would be to write the `BIP`, so we decided that's where we would spend the next amount of time just kind of refining the idea. Then after writing the `BIP`, I started working on an implementation for `Bitcoin Core`.
...
Yeah, we hadn't really met each other before. So at one point Ruben was, it's going to be hard to coordinate. I'm in Utrecht and I was in Amsterdam. This is perfect. So then I would catch the train down to Utrecht about once a week and we'd sit in a Starbucks.

**Ruben Somsen:** 00:04:11

Yep, Talk for hours.
I think, especially, so the funny thing is even though the previous episode, which by the way is episode 58, although it's very accurate, there were still a bunch of details that were very tricky to work out. We're not really going to go into them, for this discussion, because they're not really relevant for understanding the protocol necessarily but there was still a lot of work to be done, and we had a lot of pitfalls, like malleability issues, things like that. So there was a lot of work left, even though it seemed like it was pretty cut and dry, it certainly wasn't.

**Aaron van Wirdum:** 00:04:56

Yeah, I just wanted to point out that it's a second grade Utrecht homegrown project, which I'm very proud of. The first one, of course, being Bitcoin Explained.
So the BIP is BIP352 and this was finalized a couple of weeks ago?
...
Any longer. It must have been two months ago now.

**Josie:** 00:05:21

Maybe it's worth mentioning because I think there's a lot of confusion about this. We were kind of considering the BIP somewhat final, maybe a few months before that, where the protocol was done and we had test vectors, but then it took about another two months for the BIP to actually get merged. Then all of the changes were just like copy edits and we actually had people starting to build implementations before the BIP was finalized, but the BIP actually merged into the repo maybe two months ago.

**Sjors Provoost:** 00:05:49

I did a little testing myself because I was maintaining a small indexer pull request on top of your pull request to see if it's compatible and then other people would have other implementations and I would run a little script that would just compare them to make sure that for our mainnet, every silent payment tweak, and I guess we'll get back to what that is the same.
The answer was usual, but there were still a few inconsistencies. Some bugs on our end and some bugs on other people's end, so it's always useful to compare. So even when the BIP is final, you still want to maybe not tattoo your silent payment address.

**Aaron van Wirdum:** 00:06:22

Well, you were kind of going to do that now, we thought because on the previous episode, you said no time for tattoos yet, but there's still no time for it. Now you're just copying out.
...
Wait for Bolt 12 because then you can get a unified QR code tattoo of Bolt 12 and `silent payments`. Unless you want to get two tattoos.

**Aaron van Wirdum:** 00:06:46

Okay, I'm going to give a one-sentence explanation of what this sort of is, and then we'll go to a short recap, but you can correct me if I'm wrong.
...
So, silent payment addresses are also something you can post publicly, but then whoever is paying you basically generates a new address for you every time, with you being the only one to be able to spend from that. Right?
...
So it's not interactive, but the recipient knows what that address is going to be. So as the sender, you don't have to send an email to the recipient saying, please look here.

**Aaron van Wirdum:** 00:07:50

Yeah, so the challenge of doing something like this is how can the sender generate an address that the recipient can then spend from? That's basically the challenge that had to be solved.

**Ruben Somsen:** 00:08:04

And only the recipient can recognize also, right? Spend from and recognize.
...
I think one distinction, the difference here is the sender generates an address that is only spendable or knowable by the recipient, but also in a way that the recipient can find it without requiring extra data on the blockchain or out-of-band communication.
...
So in a nutshell, We are reusing information that is already in the transaction to establish a shared secret and that shared secret is only valid for that single transaction, and it's a shared secret between the sender and the recipient.
The shared secret is what allows the recipient to find the outputs and then the thing that we do to make it only spendable by the recipient is there's of the output that is created, which is a taproot public key.
The output is created by combining the shared secret with the recipient spend public key.
So basically the the transaction itself becomes kind of the notification of a payment and the clip, the clever trick is to do a shared secret elliptic curve Diffie-Hellman is kind of the standard for that, that involves a private key, public key pair. So the sender is just reusing the private keys of the UTXOs that they wanna spend and then the recipient is finding that shared secret by looking at the public keys of those UTXOs.

**Aaron van Wirdum:** 00:10:18

Yeah, we're gonna break this down. So first, to start, what actually is a silent address? Like, is it basically a regular Bitcoin address?
Could you also just send money to it and then spend from the private key?
No, it's a special type of public key, right?
...
Yeah, more specifically, it's a `Bech 32M` encoding of two public keys.
...
It's the HRP, Human readable part is what HRP means.
So when you look at a Bitcoin address that starts with a BC1P, the BC stands for Bitcoin. It's like it doesn't have any meaning for the computer other than, well, a little bit, but it's human readable, so you can say, oh, this is a Bitcoin address and not a, say, testnet address and a sign-in payment address starts with SP, so you know it's a sign-in payment address.

**Aaron van Wirdum:** 00:11:14

Okay, you are actually lightrally talking about the address format, how the address starts.

**Sjors Provoost:** 00:11:18

I would say that, yes, there's two public keys, but for the purpose of explaining the shared secret, just assume it's one public key, because you don't need to know about the second one because we were trying to explain how the shared key works, right?
...
It's not from the Monero community, before that, it was already a thing.
...
So, your private key times my public key.
...
Josie's public key because he put it on the internet and Josie will know your public key because that's where the money is coming from.

**Josie:** 00:13:23

It'll be in the transaction.
So now you've created a shared secret.
You take that shared secret, you do some things to it in the protocol, and then you take that second public key from the address, and you add it to the shared secret That makes something that is findable by me, but only spendable by me.

**Aaron van Wirdum:** 00:13:41

So for those, there might be listeners that don't know this, but the general idea of the Diffie-Hellman exchange is that we both have a public key and we both have a private key by combining or Multiplying.
Multiplying one of our private keys.
So I will use my private key in combination with your public key and you can do the opposite, your private key with my public key.
...
So before we talk about that one, just maybe to make it clear, the public key that I use to find the payment is already existing in the transaction and this is kind of the important part of `silent payments`.
...
It was kind of obvious and the user had to pay more.
...
Even back then there was already the separation of there being two keys, but just for simplicity, we didn't really talk about it.
So you have a key with which you generate the shared secrets and then there's a separate key that you add to it.
The reason we have the separation of two keys is purely to have the functionality of finding out whether or not you get paid, which requires what we call the scan key and actually spending from it, which we call the spend key.
We can separate that out so you can have a device that does all the scanning, that checks, hey, did I get paid or not?
Does the generates the shared secret and then checks, is this actually a payment? 
Actually spending it, that key can just be on a hardware wallet that isn't necessarily connected to the internet, cannot really do the scanning in an efficient way.
In theory, probably they could, but in practice, that's difficult. So that's why we have this separation of having these two keys.
if you sort of forget about that separation, you could do this whole thing with a single key.
It means you can't separate out the spending from the scanning.

**Aaron van Wirdum:** 00:16:34

Yeah, as I recall in the earlier podcast, the previous podcast we did on this, you just tweaked the set, you used the same key to tweak it again.
That was to make sure that only the recipient can spend the money afterwards, you're saying that it was implemented differently because this makes it easier to scan to see if you receive the payment.
...
The last time we talked about this, I tried to keep it simple, So I didn't talk about this distinction but since this is the second time, I thought it would be good to sort of bring it up and Josie brought it up.

**Josie:** 00:17:24

I think maybe to give a concrete example, when you're the recipient, the way that you scan is by having your scan private key in a hot online device and so in the old method where we were reusing the same key, that means in order to scan, the private key that you need to find the payments and also spend the payments is now on an internet connected device.
...
We specified in the BIP that the sender always generates a taproot output.
...
There needs to be at least one input that is specified in the BIP.
...
There's complications with other inputs like the old bare multisig style, where Ruben had mentioned earlier, one of the things we were working through in the BIP was dealing with these cases of malleability.
...
So if you look at the inputs and there's nothing in there that's specified in the BIP, you're gonna skip this transaction.
...
Then you take that public key, you multiply it by your scan private key, generate the shared secret, and then you add your spend public key to it, which then creates a new public key and you see if that public key is present as any of the taproot outputs.
...
I think that's the most accurate thing to say, just one additional signature check.
...
So lightClient, it's more complicated for lightClient then, because lightClient don't usually check every transaction to see, they don't check every transaction.

**Ruben Somsen:** 00:22:57

There's different kinds of light clients, right?
...
There are better ways and we did an episode about it.
...
But the bottom line is you instead of telling somebody else what your addresses are, you ask them for sort of a summary of the block, a filter of the block, and you download these summaries, which are much shorter than blocks, And then based on that, you're able to calculate whether or not that block has any transaction that pays you or at least guess.
You might be wrong in the sense that there can be false positives.
So once you think there's a transaction for you, you download the actual block and you're going to download slightly too many blocks.
This is a nice way so that as a light client, you're only going to download a fraction of all the blocks to see if there's an intersection for you.
...
I think one useful definition that isn't really used today, but it's important to highlight is that you somehow figure out whether or not you got paid without a full node doing all the work for you, right?

**Ruben Somsen:** 00:24:58

Without your full node doing all the work for you, somebody else's full node will do it.
...
Is that a light client? Not really, I would say.
...
Exactly, but then like if that remote control full node is somebody else's, now suddenly it becomes a light client.
...
Yes, but I think today when we talk about light clients, a lot of people call the connect to somebody else's node model, they call that a light client.
...
I don't think that really is what we want from a light client at least, or what we should call a light client.
...
Yeah, I wanna mention, I don't think that it's not computationally infeasible.
...
And the second one is whether or not you figuring out whether or not you got paid, whether you did that in a private way or not, right? because the non-private way is sort of simple.
Like you give someone an address and you just ask like, hey, did I get paid? they'll say yes or no.
...
One of the big components there that Shorst mentioned is BIP 157, BIP 158, That is one component.
But the first thing we need to answer, the scanning question is how do we get a light client?
...
The answer to that is essentially that for every transaction in the Bitcoin blockchain, every silent payment eligible transaction, a full node or some kind of server can generate a single 33-byte value and send that to the light client.
And the light client can use that to generate what essentially would be the address if it was a payment for them.
...
Light clients don't have access to blocks or transactions, this 33 byte value is a summary of the input public keys of the transaction.
So it's the same information anybody would have just by looking at the blockchain.
We'll call it an indexing server, an indexing server just sees transactions coming in.
...
But this sounds to me like Light clients would still be checking every transaction in a way.
...
One important point here is that we can save a lot of bandwidth by discarding all the transactions where all the outputs were spent.
...
And the cut-through has a really nice property here where the longer, so when you first hear this, the initial thought is the longer the light client stays offline, the more data they're going to have to download.
And this has been a problem, I think, in other light clients that do this downloading stuff.
...
So if a light client were to scan once every three days, they're skipping a whole lot of transactions that are just not relevant to them.
And this is, I think, the trick that we realize actually makes light client scanning feasible.
...
What you do is you use that 32 byte number, you multiply it is with your own private key, your own silent payment private key, and then you know what address you should have been paid to.
...
I think BIP 158 is just the best example that we have right now.
...
Is that something that audio will allow you to explain or should we just refer people to the BIP?
...
So that's 33 bytes.
The first byte tells you whether it's an even or an odd point and then the next 32 bytes is an encoding of the X coordinate of the point.
...
**Ruben Somsen:** 00:35:15

Then you're just multiplying your private key with this public key that you just found. A point is a public key.
...
It kind of does in a way, because with assuming that you got that from an XPub, you now have kind of this gap limit thing to worry about.
...
So what we have is kind of this like stale data problem, where if I post payment instructions in a public place, I should probably keep checking for them.
...
It's no longer good.
Why people prefer expiry, revocation is really hard.
Like it's just a difficult problem.
But I think our energy would be better spent tackling the hard problem of revocation because that actually gets us what we want and I think that would be extremely useful.
...
It might, be a different BIP at some point.
...
## Silent Payments Incompatibility with CoinJoin
...
Yeah, the problem was more, so there was a solution and it's still the same solution, but essentially the problem is that whenever you make a payment with more than one person on the input side, So a CoinJoin is essentially not just your inputs, but somebody else's inputs also go into the transaction and the way `silent payments` work is that you need all the keys from all the inputs.
So now you have to collaborate with other.
...
You don't necessarily need the private keys themselves, but now you have other participants that have inputs.
And so you need them to collaborate with you in order to create a coin join to a silent payment output.
In order to do that, there's a naive way of doing that, which means that you're giving up the person that you're paying.
You can say to the other CoinJoin participants, hey, I want to pay this person, please help me out.
But if you do that, then now you're revealing to them who you're paying and you don't want that.

**Ruben Somsen:** 00:40:59
...
Yeah, but in one scenario, you're fine with the other participants knowing that you're making a `silent payment` and in Coinjoins, you're not fine with that.

**Josie:** 00:42:07

if you know, there's another problem, are you fine or not with them knowing who you're paying?
The second question is, are you certain that the data that they gave you is correct?
Because the very naive way would be for them to give you your private key, but that sort of defeats the purpose of multisig, we're not doing that.

**Ruben Somsen:** 00:42:28

In a CoinJoin, you can just rug them, that would be bad.
...
So whatever Bitcoin address you're seeing on your screen, the wallet's saying, I'm about to send to this Bitcoin address, trust me, it follows from the `silent payment` protocol.
It might not if the other participants do something wrong.
So you want to be able to check.

**Josie:** 00:42:55

This is even a problem on the side of hardware wallets, where the hardware wallet might malfunction or there might be some kind of issue, maybe a bit has flipped somewhere and the calculation that your hardware wallet did was actually incorrect.
...
**Ruben Somsen:** 00:43:15

So there are a bunch of cases.

**Ruben Somsen:** 00:43:18
...
And normally, you know, you look at your screen, your wallet says, I'm going to send the money to this and this address, and then the hardware wallet confirms, do you want to send to this in this address and you're good but the problem with `silent payment` is you don't know what the address is going to be because it involves the shared secret and therefore you need your own private key to figure out what the address is.
But that address is, that private key is on the hardware wallet.
So in order to know what address you're going to send to, you need to ask the hardware wallet, hey, this is the `silent payment` address of the person I'm about to send money to, tell me what the address is, and then you just have to kind of trust the hardware wallet.

**Josie:** 00:44:02

So this is what you solved.
Yeah, so there is a way to actually basically do an extra calculation when you're generating the `silent payment` address, this is called a discrete log equivalence proof.
It shows that the calculation that you did, the shared secret that you generated was correctly generated.
...
And this fixes hardware wallets, CoinJoin, everything.
...
If you care about that, you can even go as far as to like pretend that you're making a `silent payments` as to throw them off whether or not you're making a silent payment.
...
If I were to look at, you know, I'm the NSA, I know every silent payment address out there because I watch the internet, I still could not like go back and see if it matches any of them.
...
Then I think now it's gonna be its own BIP where someone suggested, hey, this discrete log equivalency stuff seems generally useful, we should probably have a separate BIP for it.
...
There are no additional assumptions over what we already have for schnorr signatures.
...
Yeah, so that's another interesting one to discuss, that I didn't think of until after we did the podcast.
In the discussions, `silent payments` are very privacy focused, but there are scenarios where you actually want less privacy and so with `silent payments`, you actually have a case where whenever when somebody sends you money repeatedly, you don't know the same person was sending you money multiple times.
...
That also helps in the case where the recipients, they go to jail or they have to reveal to the government certain amounts of data, now the government knows that you sent them multiple payments or something like that.
...
When you go to jail, you give the private keys to the police because they've been torturing you for seven days.
...
This is unique to `silent payments`.
...
With the caveat that these payments, you do some cluster analysis on them.
So there's still some of that, but at least it's not like, I gave this address to this person.

**Aaron van Wirdum:** 00:50:11

Well, let me give a more specific example.
If I had an XPub or something like that, BIP47 establishes this payment channel, And so any payment made through that payment channel is clustered already.
You don't need to do any chain analysis stuff.

**Aaron van Wirdum:** 00:50:30

Like Ruben said, there are use cases where you, you do want that, but I don't think that a protocol like silent payment should have that by default.
...
Like an elliptic curve math is you have to write a whole, you have to copy the whole Bible, whereas the, just the simpler operations, like you write one sentence.
...
What's a label then? It's just different address?
...
And then you say, ah, okay, this particular payment was made to this label that I handed to Sjors.
...
So, the labels is this really cool way where I still, I do a constant amount of work, no matter how many customers there are, but I do have a way of figuring out, okay, Sjors's just made a deposit to his account.
...
So then when you're making a payment, it will lightrally say, are you sure you want to pay to name of exchange?
...
And so this is where, you know, the silent payment address can kind of be this persistent trustless identity, which plays really nicely into this use case for exchanges, but also BIP353 now, which is like human readable payment instructions where you could have like a username that has an underlying sign up payment address And because we're dealing with public keys here, there's just a lot more trustlessness, but with better UX that we get.

## Closure

**Aaron van Wirdum:** 00:55:37
...
The reason that those aren't making progress right now is they're waiting on an upstream PR to `LibSec-P`, which is the cryptography library that we use in Bitcoin Core.
...
And also any hardware wallet, mobile wallet, whatever, can also reuse this `LibSec-P` module.
...
Cake Wallet, I think, was the first one to have a full-light client.
So they have a fully private-light client that doesn't use BIP158.
...
**Aaron van Wirdum:** 01:00:23

Sure, we good?

**Sjors Provoost:** 01:00:27
