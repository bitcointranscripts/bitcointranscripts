...
There were some exceptions, but I think neither of us really liked that, so we kind of made it the rule.
...
Because just arguing with people on Twitter or even Nostr is just a huge energy drain.
We did record a whole episode on probably the most controversial part of it.
That's our previous episode, the return of the OP_RETURN.
...
## OP_RETURN policy limit
Aaron: 00:04:58

Yeah.
Okay, well, regardless, let's actually kick off with that specific topic just to get it out of the way.
So the OP_RETURN policy limit has been lifted basically entirely, right?
Again, I think We've said enough about that.
Anyone who wants to know the details about that can go to the previous episode.
However, there is also another related change that's close to it, but that we did not discuss in the previous episode, which is, I think there's also a policy rule, is that it's now also possible, besides bigger OP_RETURNs, to include multiple OP_RETURNs in one transaction, right?
We did not cover that last time.

Sjors: 00:05:45

I'm not sure if we did, but we can just repeat it.
So you can either make one OP_RETURN and that is the maximum size of a transaction or less, or you can make multiple ones, which can ultimately add up to the same size.
...
Then somebody could make, I guess, 20 OP_RETURNs of 2 bytes each, if they somehow wanted to.
...
But the critique was, but now you can still include multiple OP_RETURNs in a transaction.
However, The point is that if you do change it back to, let's say, 83 bytes, then if you then put multiple OP_RETURN in a transaction, then their combined size can still only be 83 bytes.
So, even if you do change the defaults, for whatever reason you may think you want to do that, then that is still the actual limit for OP_RETURNs.
You can split it up into two different OP_RETURNs, but it's still 83 bytes in that case, right?
Yeah, okay.
So then why you said the change was made, and I'm talking again specifically about the fact that you can include multiple OP_RETURNs, that was made as a simplification?

Sjors: 00:08:11

Yeah, I mean, it depends on how you look at it, I guess, but The idea was there can only be one, so that restriction is dropped.
Now it doesn't matter how many outputs are OP_RETURNs.
...
If your node receives a block with a transaction in it that has bigger OP_RETURNs than whatever setting you're switching it to, then you'll still accept that.

Sjors: 00:09:05

Yeah, in fact, a block could contain a 4 megabyte OP_RETURN in a single transaction, which would not be even possible with the new Bitcoin Core over the mempool.
Yeah.
So each transaction is limited to 100 kilobytes in general.

Aaron: 00:09:18

Yeah, so what is technically new in this specific instance is that there can now be multiple OP_RETURNs in a transaction on whatever size you want to put that setting.
...
But another reason to do this is because if somebody has a need to have multiple OP_RETURNs, and I don't know if anybody does at the moment, but if they do, and we still have this limit of one OP_RETURN transaction per turn, one OP_RETURN output per transaction, then there is another incentive for people to go outside the mempool and use accelerators, which creates the centralization pressure that we tried to solve in the first place.
...
While you, and I agree with you, see it more as, well, if there's no harm, then what's the problem?
...
Anyone who was listening specifically to hear more about the OP_RETURN drama can now switch off because we're going to talk about other interesting notable improvements in Bitcoin Core 30.
...
Now, keep in mind, everything is possible with Bitcoin Core if you recompile it yourself, but in this case, it's just a matter of changing a setting.
So you can already lower it to 1,000th of a Satoshi per vByte.
...
So in this case, and also very relevant to the OP_RETURN issue, but in this case as well, Bitcoin Core has decided to allow lower fee transactions in the mempool and relate it to other nodes in order to better reflect the mempool with blocks that are being found.
Good summary?
...
We've seen it with OP_RETURN.
Now we're seeing it with the fee rate.
And the response has been slightly different in different cases.
So in the case of full RBF, you had somebody basically promoting that concept, releasing an alternative client, and then Bitcoin Core did not follow until at some point, you know, all the miners, or at least a large chunk of the miners were doing this and compact block relay was broken.
...
Okay, so then we had the OP_RETURN stuff where Bitcoin Core developers saw that a use case was imminent and tried to preempt it.
That caused an enormous amount of drama.
And now it's kind of in between, like this spontaneous change in policy by users and Bitcoin Core follows it just a month or two later.
...
Bitcoin Core can only adjust to that.
So And then, like you said, there are a couple of ways to do that, which is either sort of do it in advance when you see it coming, or only do it when it's kind of too late.
...
So then Bitcoin Core in this release said, okay, we're going to lower the fee minimum in order to better...

Sjors: 00:19:21

And this change, I don't think this change is particularly bad because the last time this limit was lowered, I think it was 10 sets per vByte in the past, or maybe five, and the price has gone up 10,000x.
...
But yeah, as far as I know, the wallet still has one sat per vByte as a minimum.
I think you can manually set it lower but my guess is the wallet will follow in the next in a later release.
You kind of want to make sure that the network actually relays your transaction because let's say we change the wallet now to use this 0.1 sat per vByte and you start your GUI and you have maybe five peers and none of them you know have this custom setting because you know a lot of people do but not everybody does so you might not be connected to anyone he does.
So you sent your 0.1 sats per vByte transaction and it just gets rejected by your peers.
...
It's because it never went anywhere because you only had your node open for five minutes and none of your peers accepted your transaction.
...
Right.
So in that sense, Bitcoin Core actually also adjusts to the situation on the network.
...
How so?
You can use RBF with Bitcoin Core, right?
...
I think it's per transaction, but what you're counting is you're counting sigops in the inputs and in the outputs.
...
Right, so it's kind of in anticipation of hopefully, potentially a future soft fork to already sort of, push network behavior kind of in that direction so that when the soft fork happens, then it's a relatively small change that can't really cause too much problems, essentially, right?
...
We do apparently keep getting back a little bit to the OP_RETURN issue.
As a general sort of direction, I think the goal of the Bitcoin Core project is to essentially, as much as possible, just get rid of policy rules and make what nodes accept and relay as close as possible to the actual consensus rules.
Is that correct?
...
I don't know if that's realistic, but one of the reasons is that one of the goals, one of the use cases for policy is upgrades, upgrade hooks.
...
So I get the philosophy of like you don't want to have these pedantic limits as some people would call them, like the OP_RETURN size or RBF things.
But I don't think it's true that consensus and policy cannot be identical because you would never be able to upgrade Bitcoin anymore.
...
Okay, what's, just to give our listeners a sort of rough idea because now the limit is 2500.
...
There is a transaction that actually has 2500 sigops. Are we talking about a transaction that's going to stall your node with like a second or not even that?
...
So my guess would be that the combination of these high numbers of sigops and a very large transaction are what really is more dangerous.
...
If you simplify it, If two transactions of 1 kilobyte have like one bad each, right, it adds up to two bad.
But if you make it a 2 kilobyte transaction, it might be four bad.
...
Right.
But in any case, to summarize this, a transaction that has 2,500 sigops is both very implausible to be used for any real reason, while at the same time also still well within safety limits.
...
## Improvement on 1-parent-1-child transactions / more robust orphanage
Aaron: 00:31:42
...
It basically, well, I don't understand the full mechanism, but it boils down to, let's say you have 10 peers, and one of your peers starts misbehaving by sending you tons and tons of orphans, you're not just going to flush them all.
...
So it has a documentation, you call `bitcoin node`, it starts `bitcoind`.
...
If it's not in the GUI I'm not aware of its existence.
...
## IPC mining interface / Stratum V2
Aaron: 00:38:44
...
And so the original idea when the spec was written was for Bitcoin Core to be the template provider.
...
Yeah, so Bitcoin Core is good at making templates, but the problem was that in order for Bitcoin Core to do this, it would have to speak the Stratum V2 protocol.
...
So that's what's in version 30 of Bitcoin Core.
...
## Added NAT PMP Import Control Protocol
Sjors: 00:45:02

NATs PMP Import Control Protocol, PCP.
...
It's always been there, but it was off because there were too many vulnerabilities in the software.
...
If the ratio becomes too asymmetrical, then the few nodes that are listening will get completely overwhelmed.
...
And not in the woke sense of the word, but in the as many places in the world like different countries, different networks, so that it's harder for one network operator to trick you.
...
Yeah, okay.
So, and this is...
Sorry, what exactly is new in Bitcoin Core?
...
So that's why some people are saying, oh, the majority of Bitcoin notes is on tor.
...
Okay, I think we made it through the list, Sjors.
...
Thanks for listening to Bitcoin, Explained.
